...
import { Plus, KeyRound, Copy, Trash2, Check } from "lucide-react";
...
export default function ReportProjectsDialog({ trigger }: { trigger: React.ReactNode }) {
  ...
  // The plaintext key shown exactly once (from create or rotate).
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-2xl">
        ...
        {revealed && <RevealedKey label={revealed.label} value={revealed.key} onDone={() => setRevealed(null)} />}
        ...
        <div className="space-y-2 max-h-72 overflow-y-auto">
          ...
          {projects.map((p) => (
            <div key={p.id} className="flex items-center gap-3 rounded-md border p-2.5 text-sm">
              ...
              {canWrite && (
                <>
                  <Button size="sm" variant="ghost" className="gap-1" onClick={() => handleRotate(p.id, p.name)}>
                    <KeyRound className="h-3.5 w-3.5" /> Rotate
                  </Button>
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => setProjectActive(p.id, !p.isActive)}
                  >
              ...
              {canDelete && (
                <Button
                  ...
                  onClick={() => deleteProject(p.id).catch((e) => toast.error(e.message))}
                >
